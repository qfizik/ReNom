...
class adagrad_update(optimizer_factory):
  ...
  class gpu_op:
    ...
    def update(self):
      for gpu, handle in rm.cuda.RenomHandlers(self.gpus):
        ndy = self._outputs[gpu].empty_like_me()
        rm.cuda.cu_optimizer_adagrad(self.learning_rate, self.epsilon, self._dy[gpu], self._prev[gpu], ndy, self._prev[gpu])
        self._outputs[gpu] -= ndy
...
class adadelta_update(optimizer_factory):
  ...
  class gpu_op:
    ...
    def update(self):
      ...
      for gpu, handle in rm.cuda.RenomHandlers(self.gpus):
        ndy = self._outputs[gpu].empty_like_me()
        rm.cuda.cu_optimizer_adadelta(dr, self.epsilon, self._pdy[gpu], self._pdx[gpu], self._dy[gpu], ndy)
        self._outputs[gpu] -= ndy
...
class adamax_update(optimizer_factory):
  ...
  class gpu_op:
    ...
    def update(self):
      ...
      for gpu, handle in rm.cuda.RenomHandlers(self.gpus):
        ndy = self._outputs[gpu].empty_like_me()
        rm.cuda.cu_optimizer_adamax(self.alpha, self.epsilon, (self.beta1, rb1), (self.beta2, rb2), self._mom1[gpu], self._mom2[gpu], self._dy[gpu], ndy)
        self._outputs[gpu] -= ndy
...
class adam_update(optimizer_factory):
  ...
  class gpu_op:
    ...
    def update(self):
      ...
      for gpu, handle in rm.cuda.RenomHandlers(self.gpus):
        ndy = self._outputs[gpu].empty_like_me()
        rm.cuda.cu_optimizer_adam(self.alpha, self.epsilon, rb1, self.beta1, rb2, self.beta2, self.min, self.time % self.CHECK_ZERO_VALUE == 0, self._mom1[gpu], self._mom2[gpu], self._dy[gpu], ndy)
        self._outputs[gpu] -= ndy
      self.rb1 = rb1 * self.beta1
      self.rb2 = rb2 * self.beta2
      self.time += 1
