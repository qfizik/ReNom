...
class l2norm_forward(operation):
  ...
  def perform(self):
    for gpu, handle in rm.cuda.RenomHandlers(self.gpus):
      norm = rm.cuda.cusum((self._inputs[gpu] * self._inputs[gpu]), handle, axis=1, keepdims=True)
...
class l2norm_backward(operation):
  ...
  def perform(self):
    for gpu, handle in rm.cuda.RenomHandlers(self.gpus):
      norm = rm.cuda.cusum((self._fwd_ins[gpu] * self._fwd_ins[gpu]), handle, axis=1, keepdims=True)
      rm.cuda.cusqrt(norm, norm)
      norm = norm + 1e-7
      dx = self._inputs[gpu] * norm - \
                rm.cuda.cusum(self._fwd_ins[gpu] * self._inputs[gpu], handle, axis=1, keepdims=True) * self._fwd_ins[gpu] / norm
      dx = dx / (norm * norm)
