...
class group_normalize(Node):
    ...
    def _oper_cpu(cls,x, w, b, G,eps):
        assert len(x.shape)==4,"Input Data for Group Normalize layer must have 4 dimensions. Provided {}.".format(len(x.shape))
        N,C,H,W = x.shape
        G = min(G,C)
    ...
    def _backward_cpu(self, context, dy, **kwargs):
        N, C, H, W = dy.shape
        G, x, x_norm, mean, var, w, b, eps = self.attrs._g,self.attrs._x,self.attrs._x_norm,self.attrs._m, self.attrs._v, self.attrs._w, self.attrs._b, self.attrs._eps
    ...
    def _backward_gpu(self,context, dy,**kwargs):
        N, C, H, W = dy.shape
        G, x, x_norm, mean, var, w, b, eps = map(get_gpu,(self.attrs._g,self.attrs._x,self.attrs._x_norm,self.attrs._m, self.attrs._v, self.attrs._w, self.attrs._b, self.attrs._eps))
        ...
        if isinstance(self.attrs._x,Node):
            ...
            dvar = rm.sum(dx_groupnorm * -1.0 / 2 * (x_group - mean) / (var + eps) ** (3.0 / 2), axis=(2,3,4), keepdims=True)
